...
HOST = os.getenv('NAS_HOST', '175.126.187.60')
USER = os.getenv('NAS_USER', 'pdi1972')
PASS = os.getenv('NAS_PASS', 'timess9746')
REMOTE_BASE_PATH = '/srv/dev-disk-by-uuid-4b8dbcea-7516-4842-89b9-14865912be78/docker/gundammap'
    ...
    try:
        ...
        while not channel.exit_status_ready():
            if time.time() - start_time > timeout:
                ...
                # 타임아웃 시 강제로 스트림 읽기 시도 후 종료
                break
            
            # 버퍼에 데이터가 있는지 확인하고 읽기
...
def deploy():
    ...
    files_to_deploy = [
        ...
        ('gundammap/settings.py', 'gundammap/settings.py'),
    ]
    ...
    try:
        ...
        if success_count > 0:
            ...
            # Django migrations 실행 (타임아웃 적용)
            logger.info("\nRunning migrations...")
            migrate_cmd = f"echo {PASS} | sudo -S docker exec gundammap-backend python manage.py migrate --noinput"
            exit_status, output, error = exec_command_with_timeout(client, migrate_cmd, timeout=60)
            if exit_status == 0:
                logger.info("Migrations completed")
            else:
                logger.warning(f"Migration warning: {error}")
            
            # 컨테이너 재시작 (타임아웃 적용)
            logger.info("\nRestarting web container...")
            restart_cmd = f"echo {PASS} | sudo -S docker restart gundammap-backend"
