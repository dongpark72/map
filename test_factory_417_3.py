...
def test_factory_api(pnu, target_jibun):
    sigungu_cd = pnu[:5]
    ...
    print(f"Checking {len(months)} months for Sigungu: {sigungu_cd}, Target: {target_jibun}")
                ...
                if res.status_code == 200:
                    if "<item>" in res.text:
                        ...
                        for item in items:
                            ...
                            if target_jibun in jibun and '감전동' in umd:
    ...
    if not found_any:
        print(f"No transactions found for {target_jibun} in the last 24 months.")

if __name__ == "__main__":
    # 부산 사상구 감전동 417-3
    pnu = "2647010200104170003"
    test_factory_api(pnu, "417-3")
