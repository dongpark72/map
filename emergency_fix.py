...
# Pattern to find the broken showLandInfo function and replace it with a clean one
import re

# Clean version of showLandInfo
clean_js = """        async function showLandInfo(pnu) {
...
# Use a very broad regex to capture the entire broken function
pattern = re.compile(r'async function showLandInfo\(pnu\)\s*\{.*?\}\s*function toggleInfoPanel\(\)', re.DOTALL)
# We need to make sure we don't eat toggleInfoPanel
content = pattern.sub(clean_js + "\n\n        function toggleInfoPanel()", content)

# 2. Fix the missing title in header
header_broken = '<div class="info-panel-header">\s+<div class="header-btns">'
header_fixed = '<div class="info-panel-header">\n            <div class="info-panel-title">토지 정보</div>\n            <div class="header-btns">'
content = re.sub(header_broken, header_fixed, content)

# 3. Double check CSS for info-panel-title
if '.info-panel-title {' not in content:
    content = content.replace('.info-panel-header {', '.info-panel-title {\n            font-size: 0.95rem;\n            font-weight: 700;\n            color: #2563eb;\n        }\n\n        .info-panel-header {')

with open(file_path, 'w', encoding='utf-8', newline='\n') as f:
    f.write(content)

print("JS Syntax and Header Fixed")
