...
DEST_DIR = r"e:\Antigravity\Gundammap\backups\v2.3"
EXCLUDE_DIRS = {
    'backups', '.git', '__pycache__', '.idea', '.vscode', 'venv', 'node_modules', 
    'static', 'media', 'staticfiles' # Assuming strict source code backup, but maybe static/media are needed? 
    # v2.2 list didn't show static/media, but let's check. 
    # I'll enable static if it exists, but usually source doesn't have generated static.
    # The user's goal is a code backup.
}
# 'static' might be important if it contains source css/js. 
# Looking at previous file lists, I saw 'luLandDet.js' in root.
# I will exclude standard heavy dirs but keep others.

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger("BackupV2.3")
...
def backup_project():
    ...
    if all_critical_present:
        print("SUCCESS: Full backup to v2.3 completed successfully.")
    else:
